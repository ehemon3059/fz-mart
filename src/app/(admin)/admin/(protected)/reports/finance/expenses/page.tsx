...
import { listExpensesInRange } from "@/server/finance/expenses";
...
export const metadata = { title: "Operating Expenses — FZ-Mart Admin" };
...
export default async function ExpensesPage({
  ...
  const expenses = await listExpensesInRange(start, end);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900">Operating Expenses</h1>
          <p className="mt-1 text-sm text-gray-500">
            Manual overheads for {monthLabel} — {formatTaka(total)} across {expenses.length} entries.
          </p>
        </div>
        <div className="flex items-end gap-3">
          ...
          <Link
            href="/admin/reports/finance/expenses/new"
            ...
            {expenses.map((e) => (
              <tr key={e.id} className="hover:bg-gray-50">
                <td className="px-4 py-2 text-gray-500">
                  {e.incurredOn.toLocaleDateString("en-BD")}
                </td>
                ...
                <td className="px-4 py-2 text-right">
                  <div className="flex items-center justify-end gap-3">
                    <Link
                      href={`/admin/reports/finance/expenses/${e.id}/edit`}
                      className="text-gray-500 hover:text-gray-900"
                      aria-label="Edit"
                    >
                      <Icon name="pencil" size={16} />
                    </Link>
                    <DeleteButton action={removeExpense} id={e.id} label="expense" />
                  </div>
                </td>
              </tr>
            ))}
