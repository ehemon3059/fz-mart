"use client";

import { useRef, useState, useTransition } from "react";
import { Icon } from "@/components/icons";
import { saveCourierSettings, testCourierSettings } from "./actions";
import { COURIER_COPY, type CourierLang } from "./content";
import {
  ActiveProviderSelector,
  PathaoForm,
  RedxForm,
} from "./ProviderForms";

const CARD_ICONS = ["warn", "globe", "settings", "shield"] as const;

interface Props {
  config: { provider: string; apiUrl: string } | null;
  pathao: {
    storeId: string;
    senderName: string;
    senderPhone: string;
    mode: "sandbox" | "live";
  } | null;
  redx: { pickupStoreId: string; senderName: string; senderPhone: string } | null;
  activeProvider: string | null;
  configured: { steadfast: boolean; pathao: boolean; redx: boolean };
}

export default function CourierPageClient({
  config,
  pathao,
  redx,
  activeProvider,
  configured,
}: Props) {
  const [lang, setLang] = useState<CourierLang>("en");
  const t = COURIER_COPY[lang];

  return (
    <div className="font-manrope mx-auto max-w-[1080px] px-4 py-8 pb-28 sm:px-7 lg:pb-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-extrabold tracking-tight text-stone-900 sm:text-[26px]">
            {t.heading}
          </h1>
          <p className="mt-1 text-[13.5px] text-stone-500 sm:text-[14.5px]">
            {config ? t.connected(config.provider) : t.notConfigured}
          </p>
        </div>

        <button
          type="button"
          onClick={() => setLang((l) => (l === "en" ? "bn" : "en"))}
          className="inline-flex items-center gap-1.5 rounded-xl border border-stone-200 bg-white px-3.5 py-2 text-[13px] font-semibold text-stone-700 shadow-soft hover:bg-stone-50"
        >
          <Icon name="globe" size={15} />
          {t.toggleLabel}
        </button>
      </div>

      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {t.cards.map((card, i) => (
          <div key={i} className="rounded-xl border border-stone-200 bg-white p-4 shadow-soft sm:p-5">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-50 text-brand-600">
              <Icon name={CARD_ICONS[i]} size={18} />
            </div>
            <h3 className="mt-3 text-[14.5px] font-bold text-stone-800">{card.title}</h3>
            <p className="mt-1.5 text-[13px] leading-relaxed text-stone-500">{card.body}</p>
          </div>
        ))}
      </div>

      <div className="mt-6">
        <ActiveProviderSelector active={activeProvider} configured={configured} />
      </div>

      <div className="mt-6">
        <CourierForm config={config} t={t.form} />
      </div>

      <div className="mt-6">
        <PathaoForm config={pathao} />
      </div>

      <div className="mt-6">
        <RedxForm config={redx} />
      </div>
    </div>
  );
}

function CourierForm({
  config,
  t,
}: {
  config: { provider: string; apiUrl: string } | null;
  t: (typeof COURIER_COPY)[CourierLang]["form"];
}) {
  const formRef = useRef<HTMLFormElement>(null);
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);
  const [testOk, setTestOk] = useState(false);
  const [pending, startTransition] = useTransition();
  const [testing, startTesting] = useTransition();

  function handleSubmit(formData: FormData) {
    setError(null);
    setSuccess(false);
    setTestOk(false);
    startTransition(async () => {
      const result = await saveCourierSettings(formData);
      if (result?.error) {
        setError(result.error);
      } else {
        setSuccess(true);
      }
    });
  }

  function handleTest() {
    if (!formRef.current) return;
    setError(null);
    setSuccess(false);
    setTestOk(false);
    const formData = new FormData(formRef.current);
    startTesting(async () => {
      const result = await testCourierSettings(formData);
      if (result?.error) {
        setError(result.error);
      } else {
        setTestOk(true);
      }
    });
  }

  const inputClass =
    "w-full rounded-xl border border-stone-200 px-3.5 py-2.5 text-[14px] text-stone-800 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100";

  return (
    <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-soft sm:p-6">
      <h2 className="text-[15px] font-bold text-stone-800">{t.heading}</h2>
      <p className="mt-1 text-[13px] text-stone-400">{t.subtitle}</p>

      <form ref={formRef} action={handleSubmit} className="mt-4 max-w-md space-y-4">
        {/* Provider is fixed for this section (Steadfast). The active-provider
            selector above chooses which courier is the default; this form only
            holds Steadfast's credentials, so a picker here was misleading. */}
        <input type="hidden" name="provider" value="steadfast" />

        <div>
          <label className="mb-1 block text-[13px] font-semibold text-stone-700">{t.apiUrlLabel}</label>
          <input name="apiUrl" defaultValue={config?.apiUrl} placeholder={t.apiUrlPlaceholder} className={inputClass} />
          <p className="mt-1 text-[12px] text-stone-400">{t.apiUrlHelp}</p>
        </div>

        <div>
          <label className="mb-1 block text-[13px] font-semibold text-stone-700">{t.apiKeyLabel}</label>
          <input
            name="apiKey"
            type="password"
            autoComplete="off"
            placeholder={config ? t.apiKeyKeepPlaceholder : t.apiKeyPlaceholder}
            className={inputClass}
          />
        </div>

        <div>
          <label className="mb-1 block text-[13px] font-semibold text-stone-700">{t.secretKeyLabel}</label>
          <input
            name="secretKey"
            type="password"
            autoComplete="off"
            placeholder={config ? t.secretKeyKeepPlaceholder : t.secretKeyPlaceholder}
            className={inputClass}
          />
        </div>

        <div>
          <label className="mb-1 block text-[13px] font-semibold text-stone-700">{t.webhookLabel}</label>
          <input
            name="webhookSecret"
            type="password"
            autoComplete="off"
            placeholder={config ? t.webhookKeepPlaceholder : t.webhookPlaceholder}
            className={inputClass}
          />
          <p className="mt-1.5 text-[12px] text-stone-400">
            {t.webhookHelpPrefix}{" "}
            <code className="rounded bg-stone-100 px-1.5 py-0.5 font-mono text-[11.5px] text-stone-600">
              /api/webhooks/courier
            </code>
          </p>
        </div>

        {error && <p className="text-[13px] text-red-600">{error}</p>}
        {testOk && (
          <p className="flex items-center gap-1.5 text-[13px] font-medium text-brand-600">
            <Icon name="check" size={14} strokeWidth={2.6} /> {t.testOk}
          </p>
        )}
        {success && (
          <p className="flex items-center gap-1.5 text-[13px] font-medium text-brand-600">
            <Icon name="check" size={14} strokeWidth={2.6} /> {t.saved}
          </p>
        )}

        <div className="flex flex-wrap gap-2.5">
          <button
            type="button"
            onClick={handleTest}
            disabled={testing || pending}
            className="flex items-center justify-center gap-1.5 rounded-xl border border-stone-200 bg-white px-5 py-2.5 text-[14px] font-semibold text-stone-700 shadow-soft hover:bg-stone-50 disabled:opacity-50"
          >
            <Icon name="globe" size={16} />
            {testing ? t.testing : t.testConnection}
          </button>
          <button
            type="submit"
            disabled={pending || testing}
            className="flex items-center justify-center gap-1.5 rounded-xl bg-brand-600 px-5 py-2.5 text-[14px] font-semibold text-white shadow-sm hover:bg-brand-700 disabled:opacity-50"
          >
            <Icon name="save" size={16} />
            {pending ? t.saving : t.save}
          </button>
        </div>
      </form>
    </div>
  );
}
