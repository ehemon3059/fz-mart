...
import { SMS_COPY, type SmsLang } from "./content";

const CARD_ICONS = ["info", "warn", "globe", "settings"] as const;

interface Props {
  config: { apiUrl: string; senderId: string } | null;
}

export default function SmsPageClient({ config }: Props) {
  const [lang, setLang] = useState<SmsLang>("en");
  const t = SMS_COPY[lang];
  ...
  return (
    <div className="font-manrope mx-auto max-w-[1080px] px-4 py-8 pb-28 sm:px-7 lg:pb-8">
      ...
      <div className="mt-6">
        <SmsForm config={config} t={t.form} />
      </div>
    </div>
  );
}
...
function SmsForm({
  ...
  config: { apiUrl: string; senderId: string } | null;
  t: (typeof SMS_COPY)[SmsLang]["form"];
}) {
  ...
  return (
    <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-soft sm:p-6">
      ...
      <form action={handleSubmit} className="mt-4 max-w-md space-y-4">
        ...
        <div>
          <label className="mb-1 block text-[13px] font-semibold text-stone-700">{t.senderIdLabel}</label>
          <input
            name="senderId"
